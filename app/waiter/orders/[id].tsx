        ...
        <ScrollView>
          ...
          {order.last_status.status == 'PLACED'
            ? <>
              <Button
                mode='contained'
                onPress={() => updateOrderStatus({orderId: order.id, orderStatus: 'CANCELED'}).then(() => router.navigate('/waiter/orders'))}
              >
                Cancelar
              </Button>
            </>
          ...
          {order.last_status.status == 'PICKED_UP'
            ? <>
              <Button
                mode='contained'
                onPress={() => updateOrderStatus({orderId: order.id, orderStatus: 'DELIVERED'}).then(() => router.navigate('/waiter/orders'))}
              >
                Entregado
              </Button>
            </>
