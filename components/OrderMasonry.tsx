...
type ValidPaths = '/cashier/orders/[id]' | '/waiter/orders/[id]' | '/chef/orders/[id]';
type Props = {
  orders: Order[],
...
const OrderMasonry = ({
  ...
  return (
    <MasonryFlashList
      ...
      renderItem={({item}) => (
        <TouchableOpacity
          onPress={() => router.push({ pathname: targetPath, params: { id: item.id } })}
        ...
        >
          <OrderCard
            order={item}
            key={item.id}
            className={(inactive ? 'opacity-60' : '') + ' ' + (orderCardClassName || '')}
